...
LESSONS = [
    {"id": "lesson01-intro",      "zip": "act-courseware-ch1-绪论-v2.21.zip",
     "title": "第一章 绪论",
     "desc": "先导概述、开环/闭环/复合控制、系统组成与分类、基本要求（稳·快·准）、课程地图；含 2 页课堂互动题。2026-09-13 起含全局主题更新（结论条贴近页脚不再遮挡内容、小视口舞台居中不裁底），章节内容未变；2026-09-21 起统一版本号 V2.21。"},
    {"id": "lesson02-math-model", "zip": "act-courseware-ch2-数学模型-v2.21.zip",
     "title": "第二章 控制系统的数学模型",
     "desc": "微分方程、拉氏变换、传递函数、方框图等效化简、信号流图、梅逊公式；含方框图化简、方框图↔信号流图转换、梅逊公式数回路三个重点交互动画页，另有 4 页课堂互动题。历史修复：信号流图支路箭头统一画在传输线中点（P40-P47）；2026-09-13 起含全局主题更新，章节内容未变；2026-09-21 起统一版本号 V2.21。"},
    {"id": "lesson03-timedomain", "zip": "act-courseware-ch3-时域分析-v2.21.zip",
     "title": "第三章 时域分析法",
     "desc": "典型输入与性能指标、一阶/二阶系统响应、劳斯判据与稳定性、稳态误差；含二阶系统参数-特征根-响应曲线联动等重点交互动画。V2.17（2026-09-14）：P6 抛物线按解析式精确绘制、特征点落在线上、图内公式改 MathJax；P14 稳态误差定义换行修复、指标徽章补峰值时间 tp；P25 求根公式补 ζ≥1 适用条件；P27-30 四种阻尼响应页左上卡统一为 C(s)→极点→h(t)→备注顺序并补全 C(s) 表达式。V2.20（2026-09-20）：P36 互动实验改为左右等大双图版式（滑块移至顶部、指标读数嵌入响应曲线空白处），修复拖动 ωn 极点不动的缺陷（极点平面改固定比例尺）；P39 系统结构图放大并改规范分式；P51/52/54/60/66/72/77/79 八页留白填充与字号放大。历史修复：SVG 图内文字统一 STIX2 公式字体、标注避线与字号放大、箭头方向修正、mk() parent 缺陷修复（Astra 修复版融入）；2026-09-13 起含全局主题更新。"},
    {"id": "lesson04-freq",       "zip": "act-courseware-ch4-频率响应-v2.21.zip",
     "title": "第四章 频率响应法",
     "desc": "频率特性与极坐标图、对数坐标图（Bode）、映射原理与 Nyquist 判据、稳定裕度、等 M/等 N 圆、Nichols 图、三频段理论、频域指标与时域指标关系；含 s 域→F(s) 域映射联动动画、典型环节极坐标图逐点绘制动画、等 M 圆滑块实验、ζ 联动实验、三频段高亮等交互动画。V2.14（2026-09-13 融合外部精修版）：全 87 页终审修复——图内有效字号全章 ≥14px、P25/P41 改 2×4 分组切换、P43-46/P55-60 样板版式统一、映射动画轨迹紧贴端点、结论条位置全局下移、小视口舞台居中修复等。"},
    {"id": "lesson05-root-locus", "zip": "act-courseware-ch5-根轨迹法-v2.21.zip",
     "title": "第五章 根轨迹法",
     "desc": "根轨迹基本概念、绘制规则七条、广义根轨迹（参数根轨迹/零度根轨迹）、根轨迹分析方法（条件稳定、主导极点、偶极子、零点影响）；61 页，全部根轨迹由数值计算严谨绘制，含连贯生长动画 + 分步演示 + 图面缩放，以及 3 页课堂互动题。V2.19：四轮细调版（13 页精修、四个部分封面补全主题配图）；2026-09-21 起统一版本号 V2.21。"},
    {"id": "lesson06-correction", "zip": "act-courseware-ch6-系统校正-v2.21.zip",
     "title": "第六章 控制系统的校正",
     "desc": "校正概念与方式、常用校正装置（超前/滞后/滞后-超前/PID）、频率法串联校正（三频段理论、超前 6 步与滞后 6 步设计及完整例题）、局部反馈校正、复合校正（按输入/按扰动补偿）；48 页，全部 Bode 图数值严谨绘制，含超前网络 α 联动、设计步骤分步演示等交互动画，另有 2 页课堂互动题。V2.21（2026-09-21）：首次发布（一轮修改版，41 页精修 + 四个部分封面配图）。"},
]
